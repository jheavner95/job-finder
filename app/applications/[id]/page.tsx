...
export default async function ApplicationDetailPage({
  ...
  const pendingFollowUps = application.followUps.filter((item) => !item.completedAt);
  const nextInterview = application.interviews.find((item) => item.scheduledAt >= new Date());
  ...
  return (
    <WorkspaceLayout className="application-detail-page">
      ...
      <section className="application-command-bar" aria-labelledby="current-stage-title">
      ...
      <div className="application-detail-summary">
        ...
        <aside>
          ...
          <section><p className="eyebrow">Next interview</p>{nextInterview ? <><strong>{nextInterview.round}</strong><time>{fullDate(nextInterview.scheduledAt)}</time></> : <p>No interview scheduled.</p>}</section>
      ...
      <section className="application-workspace-grid">
        <details open>
          ...
          <div className="application-detail-panel">
            {pendingFollowUps.map((followUp) => (
              <article className="follow-up-row" key={followUp.id}><div><strong>{followUp.description}</strong><small>{followUp.type} · {fullDate(followUp.dueAt)}</small></div><form action={completeFollowUpAction}><input type="hidden" name="followUpId" value={followUp.id} /><button className="source-run">Complete</button></form></article>
            ))}
            <form action={addFollowUpAction} className="application-inline-form">
              <input type="hidden" name="applicationId" value={application.id} />
              <label>Reminder type<select name="type"><option>Follow up</option><option>Thank-you</option><option>Check status</option></select></label>
        ...
        <details>
          ...
          <div className="application-detail-panel">
            {application.interviews.map((interview) => <article className="record-card" key={interview.id}><strong>{interview.round}</strong><span>{interview.type} · {fullDate(interview.scheduledAt)}</span>{interview.participants && <p>{interview.participants}</p>}{interview.preparationNotes && <small>{interview.preparationNotes}</small>}</article>)}
