...
export default async function ScanPage({
  ...
  return (
    <WorkspaceLayout className="scan-page">
      <PageHeader title="Scans" subtitle="What discovery has run, what it found, and how to run it now." />
      <section className={`automation-header ${nextSchedule?.nextRunAt ? "automation-enabled" : "automation-manual"}`}>
        ...
        <Link className="secondary-button button-link" href="/system/schedules">Configure schedule</Link>
      </section>
      ...
      <section className="scan-history">
        ...
        <div className="history-card-list">{batches.map((batch) => (
          <Link href={`/system/scans?batchId=${batch.id}`} key={batch.id} className={params.batchId === batch.id ? "selected" : ""}>
